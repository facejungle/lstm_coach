...
class DataLoader():
    ...
    def get_instruments_from_csv(self, exchange: str):
        ...
        instruments = new_thread(
            read_csv_async, [instruments_path, True], True)
    ...
    def get_volume_instruments(self, exchange) -> DataFrame | None:
        ...
        res_path = exchange
        instruments = new_thread(read_csv_async, [res_path, True], True)
        try:
    ...
    def instruments_to_csv(self, instruments: DataFrame, filepath: str):
        ...
        file = new_thread(read_csv_async, [res_path, True], True)
    ...
    def get_exchanges_list(self):
        ...
        return [name.split('.')[0] for name in os.listdir(exchanges_path) if name.endswith('.py') and name != "__init__.py"]
